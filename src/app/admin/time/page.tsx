...
export default function TimePage() {
  ...
  const byEntrepreneur = logs
    .filter(l => l.business)
    .reduce<Record<string, { name: string; hours: number }>>((acc, l) => {
      const id = l.business!.id;
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem", maxWidth: "1040px" }}>
      ...
      {/* By quarter + by entrepreneur */}
      {logs.length > 0 && Object.keys(byQuarter).length > 0 && (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: "1rem" }}>
          <div className="card" style={{ padding: "1rem 1.25rem" }}>
            <p style={{ fontSize: "0.65rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", color: "var(--color-text-muted)", marginBottom: "0.75rem" }}>Hours by quarter</p>
            <div style={{ display: "flex", flexDirection: "column", gap: "0.4rem" }}>
              {Object.entries(byQuarter).sort(([a],[b]) => a.localeCompare(b)).map(([q, h]) => {
                const maxH = Math.max(...Object.values(byQuarter));
                return (
                  <div key={q} style={{ display: "flex", alignItems: "center", gap: "0.6rem" }}>
                    <span style={{ fontSize: "0.75rem", color: "var(--color-text-muted)", minWidth: "72px" }}>{q}</span>
                    <div style={{ flex: 1, background: "var(--color-surface)", borderRadius: "3px", height: "6px", overflow: "hidden" }}>
                      <div style={{ width: `${Math.round((h / maxH) * 100)}%`, height: "100%", background: "var(--color-dome-gold)", borderRadius: "3px" }} />
                    </div>
                    <span style={{ fontSize: "0.8rem", fontWeight: 700, color: "var(--color-dome-gold)", minWidth: "36px", textAlign: "right" }}>{h}h</span>
                  </div>
                );
              })}
            </div>
          ...
          {Object.keys(byEntrepreneur).length > 0 && (
            <div className="card" style={{ padding: "1rem 1.25rem" }}>
              <p style={{ fontSize: "0.65rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", color: "var(--color-text-muted)", marginBottom: "0.75rem" }}>Hours by entrepreneur</p>
              <div style={{ display: "flex", flexDirection: "column", gap: "0.4rem", maxHeight: "220px", overflowY: "auto" }}>
                {Object.values(byEntrepreneur).sort((a, b) => b.hours - a.hours).map(({ name, hours }) => {
                  const maxH = Math.max(...Object.values(byEntrepreneur).map(e => e.hours));
                  return (
                    <div key={name} style={{ display: "flex", alignItems: "center", gap: "0.6rem" }}>
                      <span style={{ fontSize: "0.75rem", color: "var(--color-text-muted)", minWidth: "120px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{name}</span>
                      <div style={{ flex: 1, background: "var(--color-surface)", borderRadius: "3px", height: "6px", overflow: "hidden" }}>
                        <div style={{ width: `${Math.round((hours / maxH) * 100)}%`, height: "100%", background: "var(--color-river-blue)", borderRadius: "3px" }} />
                      </div>
                      <span style={{ fontSize: "0.8rem", fontWeight: 700, color: "var(--color-limestone)", minWidth: "36px", textAlign: "right" }}>{hours}h</span>
                    </div>
                  );
                })}
              </div>
